...
```{r}
set.seed(17) # set seed, then running the same sampling code again will give the same rows
random_sample <- kingco_sales %>%
  sample_n(3)
random_sample
```
...
# keep only Seattle properties
seattle_summary <- kingco_homes_clean %>%
  filter(city == "SEATTLE") %>%                         
# Add total_val_per_sqft
  mutate(total_val_per_sqft = (land_val + imp_val) / sqft) %>% 
# compute value per sqft
  group_by(condition) %>%                                      
# group by property condition
  summarize(
...
set.seed(50)
sample_means <- replicate(
  200,  # we repeat the process for 200 times
  bellevue_after2020 %>%
...
# With a smaller sample size (n = 10) and fewer repetitions (200), the sampling distribution of the mean still looked roughly bell-shaped and centered close to the population mean, but it was chunkier (less smooth) and showed wider spread.
